          ...
          {node.goto ? (
            ...
            >
              ...
              <span className="md:hidden flex flex-col items-center gap-1 w-full max-w-[60px]">
                {node.icon && PIXEL_ICONS[node.icon]
                  ? PIXEL_ICONS[node.icon]
                  : null}
                <span className="text-[8px] uppercase tracking-wider font-pixel opacity-70 w-full text-center truncate px-0.5 block">
                  {node.label}
                </span>
              </span>
            </Link>
            ...
            <>
              ...
              <span className="md:hidden flex flex-col items-center gap-1">
                {node.icon && PIXEL_ICONS[node.icon]
                  ? PIXEL_ICONS[node.icon]
                  : null}
                <span className="text-[8px] uppercase tracking-wider font-pixel opacity-70">
                  {node.label}
                </span>
              </span>
            </>
...
export default function CrystalDock() {
  ...
  return (
    <div className="fixed z-50 md:bottom-10 md:left-1/2 md:-translate-x-1/2 md:max-w-fit md:px-4 max-md:top-6 max-md:left-1/2 max-md:-translate-x-1/2 max-md:w-auto max-md:max-w-[95vw]">
      <div
        ref={dockRef}
        className={`relative flex items-center justify-center gap-2 bg-[#050505]/80 backdrop-blur-2xl transition-all duration-1000 ease-out ${
          isVisible
            ? "opacity-100 translate-y-0"
            : "opacity-0 translate-y-20 pointer-events-none"
        } max-md:p-3 max-md:px-5 max-md:border max-md:border-white/10 max-md:rounded-2xl max-md:shadow-[0_10px_30px_-5px_rgba(0,0,0,0.5)] md:p-2 md:px-3 md:border md:border-white/5 md:rounded-2xl md:shadow-[0_20px_40px_-10px_rgba(0,0,0,0.5),inset_0_-1px_0_rgba(255,255,255,0.1)] md:hover:shadow-[0_25px_50px_-12px_rgba(0,0,0,0.7),inset_0_-1px_0_rgba(255,255,255,0.2),0_0_0_1px_rgba(255,255,255,0.05)] md:hover:scale-[1.01]`}
      >
