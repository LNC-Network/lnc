export async function animate(canvas: HTMLCanvasElement) {
  ...
  const LNC_COLORS = ["#dd3bb7", "#b762de", "#7e27c2", "#000085"];
  type Dot = {
    x: number;
    y: number;
    targetX: number;
  ...
  const scale =
    Math.min(canvas.width / img.width, canvas.height / img.height) * 0.7;
  ...
  function loop() {
    ...
    for (const dot of dots) {
      ...
      if (dist < repulsionRadius && dist > 0) {
        ...
        const force =
          ((repulsionRadius - dist) / repulsionRadius) * repulsionForce;
