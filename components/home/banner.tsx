...
const Banner = ({
  ...
  useEffect(() => {
    const handleScroll = throttle(() => {
      ...
      scrollDirection.current = currentScroll > lastScrollPos.current ? 'down' : 'up';
  ...
  const isInBannerSection = scrollPos >= MODEL_SECTION_HEIGHT && scrollPos < TOTAL_HEIGHT;
  ...
  const translateY = modelOffset.y + (viewportHeight * 0.2 - modelOffset.y) * progress;
  ...
  return (
    <>
      ...
      <section 
        ...
        `}
        style={{
          top: isInModelSection ? `${scrollPos}px` : 
               isPastBannerSection ? `${TOTAL_HEIGHT}px` : '0',
          perspective: '2500px',
      ...
      >
        <div
          ...
          className={`
            ...
            ${progress > 0.5 ? 'cursor-pointer shadow-2xl border-2 border-blue-900 rounded-lg' : ''}
