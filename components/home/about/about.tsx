...
// ✅ Memoized Spline so it won’t re-render with every state change
const MemoizedSpline = React.memo(Spline);
    ...
    >
      ...
      <div className="flex flex-col justify-center p-8 mb-16 w-full max-w-screen-xl text-left">
        ...
        <p className="font-inter font-bold text-[20px] sm:text-[24px] md:text-[28px] lg:text-[32px] leading-relaxed mt-4 max-w-[90%]">
          <strong>LNC</strong> is a fresh, dynamic tech community dedicated to reshaping how
          students and professionals connect, learn, and innovate.
        </p>

        <p className="font-inter font-bold text-[20px] sm:text-[24px] md:text-[28px] lg:text-[32px] leading-relaxed mt-6 max-w-[90%]">
          We offer a collaborative platform where tech enthusiasts can sharpen their skills,
          work on real-world projects, and engage with a network of like-minded individuals
          passionate about technology and growth.
        </p>
      </div>

      {/* ✅ Optimized Spline Globe */}
      <div className="relative w-[90vw] max-w-[900px] aspect-[1/1] overflow-hidden rounded-full drop-shadow-[0_30px_60px_#7E27C2] animate-pulse-slow">
        <MemoizedSpline scene="/scene.splinecode" className="absolute inset-0 w-full h-full" />
      ...
      >
        {[
          ...
          { label: "Core Team Members", count: 25, color: "rgba(194, 116, 255, 1)", duration: 1600 },
          ...
          >
            ...
            >
              <Counter end={item.count} duration={item.duration} shouldStart={isVisible} />
            </p>
          </div>
        ))}
      </div>
