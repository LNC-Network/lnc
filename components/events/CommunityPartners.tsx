...
//                 <div
//                   key={`${partner.name}-${index}`}
//                   className="flex-shrink-0 w-48 h-48 mx-4 bg-gray-800 p-6 rounded-lg flex items-center justify-center text-white text-xs font-bold relative overflow-hidden"
//                 >
...
const PartnerCard = React.memo(({ partner, index }: { partner: Partner; index: number }) => {
  if (partner.logo && partner.logo !== "/placeholder.svg") {
    return (
      <motion.div
        key={`${partner.name}-${index}`}
        className="flex-shrink-0 w-48 h-48 mx-4 bg-gray-800 p-6 rounded-lg flex items-center justify-center relative overflow-hidden"
...
const CommunityPartners = () => {
  const shuffledPartners = useMemo(() => {
    const shuffleArray = (array: Partner[]) => {
      return [...array].sort(() => Math.random() - 0.5);
    };
    return shuffleArray(PARTNERS);
  }, []);
